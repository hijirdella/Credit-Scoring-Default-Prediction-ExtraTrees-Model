...
def build_customer_features_from_combined(combined_df: pd.DataFrame) -> pd.DataFrame:
    ...
    This must be consistent with the feature engineering used
    when training the ExtraTrees model.
    """
    ...
    # Loan-level aggregation (one row per loan_id per customer)
    loan_level = (
    ...
    # Loan-level default flag – aligned with final SQL version:
    ...
    # Merge demographic and behavioural features
    df_features = pd.merge(
        cust_behavior,
        cust_demo[
            [
                'customer_id', 'marital_status', 'job_type', 'job_industry',
